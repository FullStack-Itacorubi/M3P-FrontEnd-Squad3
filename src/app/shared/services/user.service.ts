...
import { IUserForm, IUserResetPassword, IUserResponse } from '../interfaces/user.interface';
...
export class UserService {
  // private _url: string = "http://localhost:3000/usuarios";
  ...
  async saveUser( user: IUserForm ): Promise<void> {
    await lastValueFrom(
      this.httpClient.post<IUserForm>(
        this._url,
        user
      )
    );
  ...
  async findUserByEmail( email: string ): Promise<IUserResetPassword> {
    return await lastValueFrom(
      this.httpClient.get<IUserResetPassword>(
        this._url + "/email",
        {
          headers: {
            email: email
